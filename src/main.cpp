...
public:
  ...
  Tile(int indexProp, int posIndex, int sizeProp) {
    ...
    if (this->index != 0) {
      std::string texturePath =
          pathToImages + "puzzle-" + std::to_string(index) + ".jpg";
      this->texture = LoadTexture(texturePath.c_str());
    }
  }

  void Unload() { UnloadTexture(this->texture); }
  ...
  void Draw() {
    ...
    DrawTextureEx(this->texture, this->realPosition, 0.0f,
                  (this->size / (float)imageSize), WHITE);
  }
  ...
  void Update() {
    ...
    if (this->inProgress) {
      ...
      this->position.x +=
          (emptySpacePos.x - this->position.x) * deltaTime * animationSpeed;
      this->position.y +=
          (emptySpacePos.y - this->position.y) * deltaTime * animationSpeed;
  ...
  void Update() {
    ...
    for (int i = 0; i < (sizeof(this->tiles) / sizeof(Tile)); i++) {
      ...
      if (this->tiles[i].index != 0) {
        ...
        Rectangle textureRect = Rectangle{
            this->tiles[i].realPosition.x, this->tiles[i].realPosition.y,
            (float)this->tiles[i].size, (float)this->tiles[i].size};

        if (this->animTileIndex != -1 &&
            this->tiles[this->animTileIndex].inProgress == false) {
          this->animTileIndex = -1;
        } else if (this->animTileIndex == -1 &&
                   CheckCollisionPointRec(inputPos, textureRect) &&
                   (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) ||
                    IsGestureDetected(GESTURE_TAP))) {
          float distance = Vector2Distance(this->tiles[i].realPosition,
                                           this->tiles[0].realPosition);
...
public:
  ...
  void Update() {
    ...
    float puzzleWidth =
        (float)std::min(std::min(screenWidth, screenHeight), 900);
...
int main() {
  ...
  while ((!WindowShouldClose()) && !puzzle.tilesClass.CheckFinished()) {
    ...
    Texture2D fixedTarget = LoadTextureFromImage(img);

    BeginDrawing();

    DrawTexture(fixedTarget, 0, 0, WHITE);

    EndDrawing();

    UnloadRenderTexture(renderTarget);
    UnloadImage(img);
    UnloadTexture(fixedTarget);
  }
